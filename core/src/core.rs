...
impl Core {
    ...
    pub fn update(&mut self) -> Result<(), CoreError> {
        ...
        self.time_passed = self.last_update.elapsed().as_secs() as u32 * 1000
            + self.last_update.elapsed().subsec_millis();
            ...
            match event {
                glutin::Event::WindowEvent { event, .. } => match event {
                    ...
                    glutin::WindowEvent::Focused(focused) => {
                        self.hibernate = !focused;
                    }
                    glutin::WindowEvent::Resized(logical_size) => {
                        self.handle_resize((*logical_size).into());
                    }
                    ...
                    glutin::WindowEvent::MouseWheel { delta, phase, .. } if !self.hibernate => {
    ...
    fn handle_mousewheel(&mut self, delta: glutin::MouseScrollDelta, phase: glutin::TouchPhase) {
        match phase {
            glutin::TouchPhase::Moved => match delta {
    ...
    fn update_title(&mut self) {
        let idle: i32 = i32::min(
            100,
            (100.
                * (1.
                    - (self.time_passed as i32 - self.sleep_time.as_millis() as i32) as f64
                        / self.time_passed as f64)) as i32,
        );
        let info = format!(
            "main thread idle = {}%, frequency = {}Hz",
            idle,
